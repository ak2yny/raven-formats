...
sound_events_m = (
    *sound_events_m,
    *(f'{e}/***RANDOM***/{i}' for i in range(20) for e in sound_events_m)
)
sound_events_v = (
    'BORED',
    'CANTGO',
...
sound_events_v = (
    *sound_events_v,
    *(f'{e}/***RANDOM***/{i}' for i in range(20) for e in sound_events_v)
)
x_voice_events = (
    'MENUS/CHARACTER/',
    'MENUS/CHARACTER/AN_',
...
def pjw_hash_reverse(sound_hash: int, events: tuple[str], znames: tuple|None = None) -> str|None:
    if znames:
        for zn in znames:
            for c in ('CHAR', 'CHARACTER'):
                for e in events:
                    hash_str = f'{c}/{zn}/{e}'
                    if (pjw_hash(hash_str) == sound_hash):
                        return hash_str
    else:
        for e in events:
            if (pjw_hash(e) == sound_hash):
                return e
...
def read_zsnd(zsnd_path: Path, output_path: Path) -> dict:
    with zsnd_path.open(mode='rb') as zsnd_file:
        ...
        for s in platform.sample_fmt.iter_unpack(zsnd_file.read(platform.sample_fmt.size * header.sample_count)):
            ...
            if platform.value in ('PC', 'XBOX', 'XENO'): sample_file_names.append(sample_file_name.upper())
    ...
    if True: # reverse generate
        zsnd_name = zsnd_path.stem.upper()
        zsfx = zsnd_name[-1]
        zsnd_names = (zsnd_name, f'{zsnd_name[:-1]}{"V" if zsfx == "M" else "M"}') if zsfx in ('V', 'M') else ()
        events = sound_events_m + sound_events_v if zsfx == 'M' else sound_events_v + sound_events_m
        music_events = tuple(f'MUSIC/{zsnd_name[:-1]}{x}' for x in ('A', 'C', 'X'))
        for so in sounds:
            if isinstance(so['hash'], int): # if the hash wasn't listed, we try to reverse generate it
                ...
                if platform.value in ('PC', 'XBOX', 'XENO'):
                    ...
                    Hash = pjw_hash_reverse(so['hash'], (
                               clean_fn, filename,
                               *(f'{e}/***RANDOM***/{i}' for i in range(20) for e in (clean_fn, filename))
                           ), zsnd_names)
                    if not Hash: # try voice and pure file name (no random)
                        Hash = pjw_hash_reverse(so['hash'], (f'VOICE/{filename}', clean_fn, filename))
                if not Hash: # if the file name isn't the hash, try character events
                    Hash = pjw_hash_reverse(so['hash'], events, zsnd_names)
                if not Hash: # if not character hash, try music events
                    Hash = pjw_hash_reverse(so['hash'], music_events)
                if not Hash and platform.value in ('PC', 'XBOX', 'XENO'): # if not music hash, try x_voice events
                    Hash = pjw_hash_reverse(so['hash'], (
                               *(f'COMMON/{e}{filename}' for e in x_voice_events),
                               *(f'COMMON/{e}{filename}/***RANDOM***/{i}' for i in range(20) for e in x_voice_events)
                           ))
                # Not checking footsteps, damagestyles, etc.
                if Hash:
                    so['hash'] = Hash
...
def write_zsnd(data: dict, output_path: Path):
    w_zsnd(
        ...
        sound_hashes = [
            ...
            for sample_index, sound in enumerate(data['sounds'])
        ]
    )

def w_zsnd(data: dict, output_path: Path, json_sounds: list[dict], json_samples: list[dict], sound_hashes: list[tuple]):
